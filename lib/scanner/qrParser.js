 ...
 * @param {string} qrData - Raw QR code data - using string for scanner compatibility
 * @returns {Object} Parsed customer data and scan type
 */
export const parseQRCode = (qrData) => {
  let customer; // Using let for variable reassignment based on QR type
  let isRedemptionQR = false; // Using boolean for binary state tracking
  let scanType = "stamp"; // 'stamp' or 'redemption' - using string literal for type identification
  ...
  try {
    ...
    if (parsedData.type === "loyalty_card" && parsedData.app === "cafe-cards") {
      ...
      customer = {
        // Using object to group customer properties for data integrity
      ...
      scanType = "redemption";
      customer = {
        // Using object to group redemption-specific properties
