 ...
 * @param {Object} profileData - The cafe profile data - using Object to group cafe configuration properties
...
export const createCafeProfile = async (profileData, cafeUserId) => {
  try {
    const profileDocument = {
      ...
      primaryColor: profileData.primaryColor || "#AA7C48", // Using hex string for color consistency
      // backgroundColor and textColor are now handled by the theme system

      // Stamp Icon
      stampIcon: profileData.stampIcon || "⭐", // Using Unicode emoji for universal compatibility
      stampIconColor: profileData.stampIconColor || "#FFD700",

      // Card Settings
      maxStampsPerCard: profileData.maxStampsPerCard || 10, // Using number for mathematical operations
      rewardDescription: profileData.rewardDescription || "Free Coffee",

      isActive: true, // Using boolean for simple state management
 ...
 * @param {string} cafeUserId - The ID of the cafe user
 * @returns {Promise<Object|null>} The cafe profile document or null - using null for graceful not-found handling
 */
